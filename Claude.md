...
| 2026-05-14 | D-019 | **표 도구 (TableShape) + Toolbar 70vw inline 확장** | tldraw 의 새 custom shape `'table'` — props (`rows`/`cols`/`cells`/`colWidths`/`rowHeights`) 모두 직렬화 가능 → 기존 broadcast sync 가 자동 처리 (별도 sync 코드 불필요). **셀 인라인 편집**: 더블클릭 → `<textarea>` 로 편집, Enter 저장 / Tab 다음 셀 / Esc 취소. **경계 드래그**: 셀 우측·하단에 6px transparent overlay → 드래그로 열 너비·행 높이 조정 (마지막 경계는 표 전체 확장, 가운데는 인접 셀 너비 재분배 + MIN_COL_W=40 / MIN_ROW_H=24 clamp). **행/열 추가·삭제**: 표 선택 시 외곽에 +/- 원형 버튼 4개 노출 (max 50행 / 20열). **코너 리사이즈**: `onResize` override 가 `resizeBox` 결과의 scaleX/scaleY 추출해서 colWidths/rowHeights 비례 scale. **Toolbar 70vw**: `CustomToolbar` 가 `DefaultToolbar` 의 `OverflowingToolbar` 임계점 (`maxItems=8 / maxSizePx=470`) 을 `20 / 1200` 으로 확장 → 와이드 스크린에서 거의 모든 도구 inline (좁은 화면은 자연 overflow). **표 버튼**: `TldrawUiMenuItem + custom SVG (TLUiIconJsx)` — tldraw built-in icon 에 grid/table 없어서. 클릭 시 viewport 중앙에 새 표 생성 + 선택. **동시성**: 두 사용자가 같은 셀 동시 편집 시 last-write-wins (D-010 정책). 다른 사용자가 셀 텍스트 수정 중 우리가 행/열 추가하면 cells 배열 길이 변경으로 일시 mismatch 가능 (broadcast 순서 정상이면 결국 수렴). **L1 abstraction**: `lib/editor/index.ts` 에 `DefaultToolbar` / `DefaultToolbarContent` / `TldrawUiMenuItem` / `StateNode` / `resizeBox` / `createShapeId` / `TLResizeInfo` 등 re-export 추가. **Google Sheets 첨부 (D-018) 와 병행** — 빠른 캔버스 메모 vs 본격 데이터 (수식/차트). 구현 메모 [`DESIGN.md` § 17.11](DESIGN.md#1711-d-019-tableshape--toolbar-70vw-구현-메모). | §5 "Could" 의 템플릿 항목을 캔버스 내 표로 구현. CLAUDE.md §13 ("MIT 호환 우선") 의 라이선스 무관 자체 shape 추가. |
