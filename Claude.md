...
| 2026-05-13 | D-018 | **Google Drive 연동 시작 — Phase 8a PoC + 8b 본격** (O-013 부분 해결) | Phase 8 의 단계적 진입. **Phase 8a (PoC, 2~3일)**: URL paste 로 Drive 공유 링크 입력 → tldraw 의 새 custom shape `gdrive-file` 로 캔버스에 아이콘 첨부 → 클릭 시 StoryWorkspace 가 split-screen 으로 iframe 패널 표시. Sheets 는 `/edit` (사용자가 Google 로그인 + 권한 있으면 inline 편집 가능), 그 외는 `/preview`. **OAuth scope 확장 / Picker / 폴더 자동 생성 / 권한 share 모두 미적용**. **Phase 8b (본격, 1~2주)**: `drive.file` scope 확장 + Google Picker SDK 도입 + 폴더 자동 생성 (`onuri-studio/{채널명 [id]}/{스토리명 [id]}/`, idempotent + 이름 변경 시 자동 rename) + 첨부는 **Shortcut** (원본 위치 보존) + 폴더 단위 `anyone with link` viewer 권한 자동 share. **익명 사용자**: Drive iframe 의 자체 권한 화면이 fallback ("Google 로그인 필요"). **Export/Import**: `.onuri.json` export 시 `fileId` 제거 + `imported: true` flag stamp, import 측은 아이콘만 표시·클릭 비활성 (Drive 연동 안 함, 별도 추후 작업). **DB**: migration 0012 (`users.gdrive_root_folder_id` / `channels.gdrive_folder_id` / `stories.gdrive_folder_id` 컬럼 + `gdrive_attachments` 매핑 테이블 + `external_integrations` 활성). 구현 메모 [`DESIGN.md` § 17.10](DESIGN.md#1710-d-018-google-drive-연동-구현-메모). | §5 "Could" Google Workspace 연계 시작. CLAUDE.md §11 Phase 8 (Google Workspace 연계) 의 첫 결정. |
