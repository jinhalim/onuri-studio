...
| 2026-05-13 | D-015 | **수정 권한 요청/승인 + 알림 inbox** (O-015 부분 해결) | **스토리 단위 / 영구 / DB 보관**. 비-owner 가 우상단 "읽기 전용" 배지 클릭 → owner 에게 `edit_request` 알림. owner 가 알림 클릭 → 허용/차단 팝업 → 허용 시 `story_permissions` 에 `role='editor'` upsert + `edit_request_approved` 알림 전송. 요청자가 그 알림 클릭 → 전체 페이지 리로드 → canEdit 재계산되어 편집 모드 진입. **Realtime push 는 broadcast 채널 `user-notifications:{userId}` 사용** (익명 사용자는 Supabase 세션이 없어 Postgres Changes 가 RLS 로 막히기 때문. 초기 fetch 도 server action + admin client 우회). 익명/Google 모두 동일 UX. 구현 메모는 [`DESIGN.md` § 17.6](DESIGN.md#176-d-015-수정-권한-요청-및-알림-시스템-구현-메모). | §5 ("Could (확장)" 의 채널 권한 시스템 일부 도입 — 스토리 단위) |
| 2026-05-13 | D-016 | **tldraw 라이선스 attribution + Editor abstraction L1** (상업화 사전 대비) | **tldraw Hobby License** 명시 (비상업적 + 워터마크). README "📜 라이선스" 섹션 + DESIGN.md § 17.7 (라이선스 가이드) + 랜딩 페이지 푸터 attribution + DESIGN.md § 17.8 (abstraction 가이드). **L1 abstraction**: `lib/editor/index.ts` 를 신설해서 tldraw 의 사용 표면 (components / hooks / shape utils / types) 을 한곳에 re-export. 9개 소비처는 `@/lib/editor` 만 import. tldraw 직접 import 는 `lib/editor/index.ts` + StudioCanvas CSS side-effect 두 곳만 남김. 미래에 editor 교체 (예: Excalidraw, 자체 canvas) 검토 시 본 파일에 동일 시그니처 adapter 만 만들면 swap 가능. **현재는 비상업** — 상업화 결정 시 SDK License 구매 또는 alternative editor 교체. | §13 ("예산 $0", "라이선스 MIT 호환 우선") 의 운영 가이드 구체화 |
| 2026-05-13 | D-017 | **Realtime sync hardening + per-story 정원 제한** | 누적된 sync 코드 정리 + 데이터 손실 quick wins + 50명 운영 대비 throttle 조정 + 정원 25명 cap. **Cleanup**: 과도한 console.log 제거 / 이중 fromUserId 안전망 제거 / `flushSave` vs `flushPendingSave` 통합 / `STATUS_DEBOUNCE_MS` 3s → 1.5s / keepalive 20s → 45s. **Quick wins (데이터 손실 차단)**: Smart autosave (다른 사용자 그릴 때 save 연기 — owner snapshot 덮어쓰기 손실 방지), Non-destructive reconnect (loadSnapshot replace → store.put merge, 로컬 unsynced 변경 보존 + catch-up broadcast), Broadcast throttle 50ms batching + record id 기반 dedupe. **50명 폴리시**: cursor 30Hz → 15Hz, laser 60Hz → 30Hz. **정원 제한**: `MAX_STORY_PRESENCES = 25` — presence sync 시 본인 포함 정원 초과면 즉시 untrack + status='overflow' + `OverflowNotice` 안내 (다시 시도 버튼 = window.location.reload). 자동 재시도 없음. 50명 운영 시점엔 Yjs CRDT 마이그레이션 필요 (DESIGN.md § 17.9). | §8 (Realtime D-010 정책) + §9 ("한 스토리당 20명") 을 25명 + Yjs 이행 경로로 갱신 |
| 2026-05-13 | D-018 | **Google Drive 연동 — Phase 8a PoC + 8b 본격 완료** (O-013 부분 해결) | Phase 8 의 단계적 진입. **Phase 8a (PoC)**: URL paste 로 Drive 공유 링크 입력 → tldraw 의 새 custom shape `gdrive-file` 로 캔버스에 아이콘 첨부 → 클릭 시 StoryWorkspace 가 split-screen 으로 iframe 패널 표시. Sheets 는 `/edit` (사용자가 Google 로그인 + 권한 있으면 inline 편집 가능), 그 외는 `/preview`. **Phase 8b**: `drive.file` scope 확장 + Google Picker SDK (`setAppId` 필수 — `NEXT_PUBLIC_GOOGLE_PROJECT_NUMBER`) + Workspace path 마이페이지 설정 + 폴더 자동 생성 (`{workspace}/{채널 [id]}/{스토리 [id]}/`, idempotent) + 첨부는 **Shortcut** (원본 위치 보존) + 폴더 단위 `anyone with link` viewer 권한 자동 share + onDelete hook 으로 cleanup. **Drive API 호출은 모두 client-side** (session.provider_token + gapi.client.drive) — 서버 측 token 암호화 불필요. **익명 사용자**: 첨부 버튼 미표시, 보기는 iframe 자체 권한 화면이 fallback ("Google 로그인 필요"). **읽기 모드 사용자 (canEdit=false)**: 모든 mime 의 iframe URL 을 `/preview` 강제 (defense in depth). **양방향 rename**: 캔버스 더블클릭 인라인 편집 → `files.update` 도 background 호출 (편집 권한 없으면 silent) + 패널 헤더 새로고침 버튼으로 `files.get(name)` 으로 Drive→캔버스 sync. **아이콘 UX**: 더블클릭 인라인 편집 + 카드 리사이즈 (코너) + 카드 크기 비례한 아이콘/폰트 스케일. **패널 닫기 = shape deselect** (재클릭 시 패널 다시 open). **StylePanel**: 선택이 전부 gdrive-file 이면 패널 통째 숨김. **Export/Import**: `.onuri.json` export 시 `fileId`/`embedUrl` 제거 + `imported: true` flag stamp, import 측은 아이콘만 표시·iframe 안내. **Frame customColor 보강**: 안쪽 fill 을 hex 의 50% lighter tint 로 채움 (`lightenHex` 헬퍼) + 정확한 `headingTextColor` 키 적용. **Split-screen 패널 너비 resize** (좌측 핸들 드래그 + iframe pointerup 흡수 차단용 fullscreen overlay). **DB**: migration 0012 (`users.gdrive_workspace_path/folder_id` + `channels/stories.gdrive_folder_id` + `gdrive_attachments` 매핑 테이블). 구현 메모 [`DESIGN.md` § 17.10](DESIGN.md#1710-d-018-google-drive-연동-구현-메모). | §5 "Could" Google Workspace 연계 시작. CLAUDE.md §11 Phase 8 의 첫 결정. |
| 2026-05-14 | D-019 | **표 도구 (TableShape) + Toolbar 70vw inline 확장** | tldraw 의 새 custom shape `'table'` — props (`rows`/`cols`/`cells`/`colWidths`/`rowHeights`/`cellMerges`/`cellStyles`) 모두 직렬화 가능 → 기존 broadcast sync 가 자동 처리. **셀 인라인 편집**: 더블클릭 (onPointerDown 으로 직접 카운팅 — tldraw 의 native dblclick suppression 우회) → `<textarea>` 편집 (Enter 저장 / Tab 다음 셀 / Esc 취소). **경계 드래그**: 셀 우측·하단 6px overlay → 열 너비·행 높이 조정 (마지막 경계는 표 전체 확장, MIN_COL_W=40 / MIN_ROW_H=24 clamp). **구조 편집**: 우클릭 컨텍스트 메뉴로 행/열 임의 위치 추가·삭제 (max 50×20). **셀 병합**: Shift+클릭 다중 선택 → 우클릭 "선택한 셀들 병합" / 병합된 셀 owner 에서 "병합 해제" — `cellMerges: { row, col, rowspan, colspan }[]` 데이터 모델, render 가 covered 셀 skip + owner 는 span 만큼 width/height 합산. 행/열 추가·삭제 시 cellMerges 자동 조정 (shift / expand / reduce, 1×1 결과는 제거). **셀별 텍스트 스타일** (`cellStyles[]`): size (sm/md/lg/xl → 11/13/17/22px) + font (sans=Pretendard / serif=Georgia / mono=JetBrains Mono) + align (left/center/right). 표 선택 시 표 위쪽 mini-toolbar (size 4개 / font 3개 / align 3개 버튼) — 활성값 표시, 여러 셀이 다른 값이면 'mixed' (어느 것도 active X). 편집 중이면 그 셀에, 범위 선택이면 범위 안 전체에 적용. **코너 리사이즈**: `onResize` override 가 colWidths/rowHeights 비례 scale. **customColor 지원**: 외곽 + 셀 구분선 색 반영. **Toolbar 70vw**: `CustomToolbar` 가 `OverflowingToolbar` 임계점 (`maxItems=8 / maxSizePx=470`) 을 `20 / 1200` 으로 확장 → 와이드 스크린에서 거의 모든 도구 inline. 표 grid picker (Excel hover 스타일) 는 `createPortal(document.body)` + `position: fixed` 좌표 → boundary item 양쪽 렌더 quirk 회피 + viewport 좌우 clamp. **표 버튼**: `TldrawUiMenuItem + custom SVG (TLUiIconJsx)`. **canEdit=true 필수**: setEditingShape 가 canEditShape 체크 통과해야 동작 (false 면 silent reject — Editor.mjs:1956). **호환성**: 옛 snapshot 의 누락 필드 (cellMerges/cellStyles) 는 loadSnapshot 전 `migrateTableShapesInSnapshot` 으로 보정 (안 하면 schema reject → store 손상 → instance state 사라짐 → 후속 render TypeError). **L1 abstraction**: `lib/editor/index.ts` 에 `DefaultToolbar` / `DefaultToolbarContent` / `TldrawUiMenuItem` / `StateNode` / `resizeBox` / `createShapeId` / `TLResizeInfo` 등 re-export 추가. **추가 fixes (동시 적용)**: tldraw Editor.dispose try/catch wrapping (v5.0.0 의 `getInstanceState() === undefined` 시 throw 우회) + StylePanel 좁은 화면 (<768px) 드래그 차단 + 모바일 오버플로 스크롤 CSS. 구현 메모 [`DESIGN.md` § 17.11](DESIGN.md#1711-d-019-tableshape--toolbar-70vw-구현-메모) + [§ 17.12](DESIGN.md#1712-d-019-후속-보강-구현-메모). | §5 "Could" 의 템플릿 항목을 캔버스 내 표로 구현. CLAUDE.md §13 ("MIT 호환 우선") 의 라이선스 무관 자체 shape 추가. |
| 2026-05-14 | D-020 | **노트 작성자 라벨 z-index 통일** | 기존 `NoteAuthorLayer` (viewport overlay, z-20) 는 노트 도형이 다른 도형에 가려져도 항상 위에 떠 있었음 — 사용자가 "메모 부분이 다른 도구 부분으로 인해서 가려지면 같이 가려지게" 요청. **해결**: `CustomNoteShapeUtilWithAuthor` (NoteShapeUtil subclass) 신설 — `component()` override 로 author 라벨을 노트의 HTMLContainer 안에 absolute 위치로 inline 렌더 → tldraw 의 shape z-index 자동 종속. 다른 도형이 노트를 덮으면 라벨도 함께 가려짐. 라벨 폰트 사이즈 / padding 은 노트의 `props.scale` 비례. customColor 동작은 base class `.configure({getCustomDisplayValues})` 로 보존 (Note 의 noteBackgroundColor / borderColor / labelColor 정상 적용). StudioCanvas 에서 `<NoteAuthorLayer />` 제거 + customShapeUtils 배열에서 기존 CustomNoteShapeUtil → CustomNoteShapeUtilWithAuthor 로 교체. customShapeUtils-shared.ts 신설 — getCustomColor 헬퍼만 분리 (customShapeUtils.ts = .ts / customNoteShapeUtil.tsx = .tsx 양쪽 import + 순환 의존성 회피). | §5 메모지 기본 요구사항 ("by 작성자" 표시) 의 시각 정확도 보강 — 버그성 수정. |
...
| ~~O-013~~ | ~~Google Workspace 통합 깊이~~ | **부분 해결: D-018 (Phase 8a PoC + 8b 본격, Picker + drive.file + Shortcut + 폴더 자동 생성)** | 양방향 동기화 / Sheets webhook 등은 별도 결정 |
| O-014 | 이메일 발신자 표기 (`noreply@` / `hello@` / `studio@`) | Phase 9 도메인 인증 시 | |
| ~~O-015~~ | ~~채널 권한 시스템~~ | **부분 해결: D-015 (스토리 단위 수정 권한 요청/승인)** | 채널 단위 권한 / 대표 이미지 등은 별도 결정 필요 |
