...
| 2026-05-13 | D-018 | **Google Drive 연동 — Phase 8a PoC + 8b 본격 완료** (O-013 부분 해결) | Phase 8 의 단계적 진입. **Phase 8a (PoC)**: URL paste 로 Drive 공유 링크 입력 → tldraw 의 새 custom shape `gdrive-file` 로 캔버스에 아이콘 첨부 → 클릭 시 StoryWorkspace 가 split-screen 으로 iframe 패널 표시. Sheets 는 `/edit` (사용자가 Google 로그인 + 권한 있으면 inline 편집 가능), 그 외는 `/preview`. **Phase 8b**: `drive.file` scope 확장 + Google Picker SDK 도입 + Workspace path 마이페이지 설정 + 폴더 자동 생성 (`{workspace}/{채널 [id]}/{스토리 [id]}/`, idempotent) + 첨부는 **Shortcut** (원본 위치 보존) + 폴더 단위 `anyone with link` viewer 권한 자동 share + onDelete hook 으로 cleanup. **Drive API 호출은 모두 client-side** (session.provider_token + gapi.client.drive) — 서버 측 token 암호화 불필요. **익명 사용자**: 첨부 버튼 미표시, 보기는 iframe 자체 권한 화면이 fallback ("Google 로그인 필요"). **Export/Import**: `.onuri.json` export 시 `fileId`/`embedUrl` 제거 + `imported: true` flag stamp, import 측은 아이콘만 표시·iframe 안내. **Split-screen 패널 너비 resize** (좌측 핸들 드래그). **DB**: migration 0012 (`users.gdrive_workspace_path/folder_id` + `channels/stories.gdrive_folder_id` + `gdrive_attachments` 매핑 테이블). 구현 메모 [`DESIGN.md` § 17.10](DESIGN.md#1710-d-018-google-drive-연동-구현-메모). | §5 "Could" Google Workspace 연계 시작. CLAUDE.md §11 Phase 8 의 첫 결정. |
